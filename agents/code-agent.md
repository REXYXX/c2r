...
2. 实现完成后必须调用 Test Agent，并只把 `{{test_agent_task}}` 作为测试任务入口。
3. Test Agent 完成后，Code Agent 只接收测试变更摘要；不要回读完整测试矩阵。
4. 测试生成完成后必须调用 Validation Agent，并只把 `{{validation_agent_task}}` 作为验证任务入口。
...
7. strict 验证失败不是完成状态；失败摘要必须作为下一轮修复输入继续处理。
...
这些文件只用于机器审计和调试。不要读取 `{{out_target}}` 或任何 Cargo target 目录。
...
- 实现某个模块时，只读取对应 compact_manifest 和一个 function_hint chunk。
- 完成一个模块或 chunk 后丢弃该上下文，再处理下一模块或下一 chunk。
- 每个模块或 chunk 完成后更新 `result/harness/context-checkpoints/code-agent.md`，下一轮只携带 checkpoint。
