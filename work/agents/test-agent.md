...
3. 若验证失败，读取 `result/harness/07-validation.json` 的 `failures` 与 `repair_required.test_agent`。
4. 对每个测试失败项，只读取对应 target manifest 和测试需求文件后修复。
5. 修复后再次交给 Validation Agent；只要 strict 未通过，就继续处理下一轮失败摘要。
6. 不得通过删除测试、减少测试需求、修改 Python validation、降低断言或改写动态 profile 来通过验证。

## 测试工作规则

- 按 target manifest 中的 `required_rust_tests` 和 `generation_items` 创建 Rust `#[test]`。
- 一次性完成当前 target manifest 对应的测试文件。
...
- Validation Agent 返回测试失败摘要时，继续修复直到 `repair_required.test_agent` 为空且 strict 通过。
